...
void line_follower_mode()
{
  while(command!="ST" and command!="m" and command!="o"){
    ...
    if (analogRead(left_ir_sensor) < 512  and digitalRead(right_ir_sensor) == LOW)
      Serial.println("Forward"), Forward(100);    // If both sensors are on white, go forward

    if (analogRead(left_ir_sensor) >= 512  and digitalRead(right_ir_sensor) == LOW)
      Serial.println("Left"), Left(100);    // Left sensor on black and right sensor on white, go left then

    if (analogRead(left_ir_sensor) < 512  and digitalRead(right_ir_sensor) == HIGH)
      Serial.println("Right"), Right(100);  // Left sensor on white and right sensor on black, go right then
